    ...
    public ResponseEntity<ApiResponse<Void>> handleValidationException(
        ...
        ex.getBindingResult().getFieldErrors().forEach(fieldError -> {
            ...
            String message = switch (fieldError.getCode()) {
                case "NotBlank", "NotNull", "NotEmpty" -> capitalize(field) + " is required";
                ...
                case "Size" -> {
                    ...
                    if (min > 0 && max < Integer.MAX_VALUE)
                        yield capitalize(field) + " must be between " + min + " and " + max + " characters";
                    else if (min > 0)
                        yield capitalize(field) + " must be at least " + min + " characters";
                    else
                        yield capitalize(field) + " must be at most " + max + " characters";
                }
                case "Min" -> capitalize(field) + " must be at least " + fieldError.getArguments()[1];
                case "Max" -> capitalize(field) + " must be at most " + fieldError.getArguments()[1];
